...
int setup() {
    ...
    ibv_srq_attr srq_attr {0};
    srq_attr.max_sge = 1;
    srq_attr.max_wr = 10;

    ibv_srq_init_attr srq_init {0};
    srq_init.attr = srq_attr;

    ibv_srq* srq = ibv_create_srq(pd, &srq_init);
    if (!srq) {
        perror("could not create SRQ");
        exit(EXIT_FAILURE);
    }
    ...
    status = ibv_post_srq_recv(srq, &rec_wr, &bad_wr);
    if (status) {
        perror("could not post to SRQ");
        exit(EXIT_FAILURE);
    }

    puts("memory posted to SRQ");
